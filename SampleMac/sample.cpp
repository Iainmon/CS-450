...
enum Colors { RED, YELLOW, GREEN, CYAN, BLUE, MAGENTA, WHITE, BLACK };
...
unsigned char *BmpToTexture(char *filename, int *width, int *height) {
  ...
  if (VERBOSE)
    fprintf(stderr, "FileHeader.bfType = 0x%0x = \"%c%c\"\n", FileHeader.bfType,
            FileHeader.bfType & 0xff, (FileHeader.bfType >> 8) & 0xff);
  ...
  int requiredRowSizeInBytes =
      4 * ((InfoHeader.biBitCount * InfoHeader.biWidth + 31) / 32);
  if (VERBOSE)
    ...
    for (int c = 0; c < InfoHeader.biClrUsed; c++) {
      ...
      if (VERBOSE)
        fprintf(stderr, "%4d:\t0x%02x\t0x%02x\t0x%02x\t0x%02x\n", c,
                colorTable[c].r, colorTable[c].g, colorTable[c].b,
                colorTable[c].a);
    }
